...
#define BG_IMG_NAME ("background.png")
...
#define ICON_SIZE           (96)
#define ICON_ROW_COUNT      (3)
#define ICON_COLUNM_COUNT   (6)
#define ICON_PAD_TOP        (100)
#define ICON_PAD_BOTTOM     (40)
#define ICON_PAD_LEFT       (115)
#define ICON_PAD_RIGHT      (115)

#define ICON_ROW_SPACE      (50)
#define ICON_COL_SPACE      (60)//((ICON_HOR_RES - (ICON_SIZE * ICON_COLUNM_COUNT)) / (ICON_COLUNM_COUNT - 1))
...
static void lv_timer_update_time(lv_timer_t * timer)
{
    ...
    lv_label_set_text_fmt(label, "   %02d:%02d:%02d  %02d-%02d-%02d", info->tm_hour, info->tm_min, info->tm_sec, (info->tm_year + 2000 - 100), (info->tm_mon + 1), info->tm_mday);
}
...
static void lcd_top_widgets(lv_obj_t * parent)
{
    ...
    lv_style_set_text_color(&label_style, red);

    lv_label_set_text(label_time, "  ");
    lv_obj_add_style(label_time, &label_style, LV_PART_MAIN);
...
static void set_menu_table_tips(lv_obj_t * parent, int count)
{
    ...
    lv_obj_align(round[0], LV_ALIGN_CENTER, -20, 250);
    ...
    lv_obj_align(round[1], LV_ALIGN_CENTER, 0, 250);
    ...
    lv_obj_align(round[2], LV_ALIGN_CENTER, 20, 250);
...
void launcher_init(void)
{
    ...
    lv_snprintf(bg_path_name, sizeof(bg_path_name), "%s%s", "/", BG_IMG_NAME);
