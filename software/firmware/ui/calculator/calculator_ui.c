 ...
 *  STATIC PROTOTYPES
 **********************/
static void calculator_tokenizer_init(char *expr); 	// 词法分析器初始化
static token_t calculator_siglechar(void);			// 获取单字符token类型
static token_t calculator_get_next_token(void);	    // 获取一个token
static token_t calculator_tokenizer_token(void);	// 得到当前token
static void calculator_accept(token_t token);		// 匹配token
static void calculator_tokenizer_next(void);		// 解析下一个token
static bool calculator_tokenizer_finished(void);    // token解析完成
static void calculator_error(error_t err);			// 错误处理
static int calculator_tokenizer_num(void);			// 将ASCII形式的数字转为数值
static int calculator_factor(void);					// 取得当前因子的值, 若当前因子(类似上式中的M)是一个表达式, 进行递归求值
static int calculator_calc_term(void);					// 求第二级优先级(乘除)表达式的值
static int calculator_expr(void);					// 求第一级优先级(加减)表达式的值
...
static void event_handler_btnmatrix(lv_event_t *e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        if (strcmp(txt, "=") == 0)                   // 执行运算
        {
            ...
            calculator_tokenizer_init(calc_exp);  // 词法分析器

            calc_results = calculator_expr();  // 求第一级优先级(加减)表达式的值
...
/*
 *  函数名：   static token_t calculator_siglechar(void)
 *  输入参数： 无
 *  返回值：   token类型
 *  函数作用： 获取单字符token类型
*/
static token_t calculator_siglechar(void)
{
    switch (*curr_char)
    {
    case '+':
...
/*
 *  函数名：   static token_t calculator_get_next_token(void)
 *  输入参数： 无
 *  返回值：   token类型
 *  函数作用： 获取一个token
*/
static token_t calculator_get_next_token(void)
{
    int i;
    ...
    // 分界符
    else if (calculator_siglechar())
    {
        next_char++;
        return calculator_siglechar();
    }
...
/*
 *  函数名：   static token_t calculator_tokenizer_finished(void)
 *  输入参数： 无
 *  返回值：   true - 无需要解析的token， false - 有需要解析的token
 *  函数作用： 判断是否token是否到达结尾
*/
static bool calculator_tokenizer_finished(void)
{
...
/*
 *  函数名：   static void calculator_error(error_t err)
 *  输入参数： 错误类型
 *  返回值：   无
 *  函数作用： 错误处理
*/
static void calculator_error(error_t err)
{
    error_code = err;
...
/*
 *  函数名：   static token_t calculator_tokenizer_next(void)
 *  输入参数： 无
 *  返回值：   无
 *  函数作用： 解析下一个token
*/
static void calculator_tokenizer_next(void)
{
    if (calculator_tokenizer_finished())
        return;

    curr_char = next_char;
    current_token = calculator_get_next_token();
...
/*
 *  函数名：   static token_t calculator_tokenizer_token(void)
 *  输入参数： 无
 *  返回值：   无
 *  函数作用： 得到当前token
*/
static token_t calculator_tokenizer_token(void)
{
...
/*
 *  函数名：   static token_t calculator_tokenizer_num(void)
 *  输入参数： 无
 *  返回值：   无
 *  函数作用： 将ASCII形式的数字转为数值
*/
static int calculator_tokenizer_num(void)
{
...
/*
 *  函数名：   static void calculator_accept(token_t token)
 *  输入参数： 需要匹配的token
 *  返回值：   无
 *  函数作用： 匹配token
*/
static void calculator_accept(token_t token)
{
    if (token != calculator_tokenizer_token())
        calculator_error(syntax_error);

    calculator_tokenizer_next();
}



/*
 *  函数名：   static int calculator_factor(void)
 *  输入参数： 无
 *  返回值：   因子的值
 *  函数作用： 取得当前因子的值, 若当前因子(类似上式中的M)是一个表达式, 进行递归求值
*/
static int calculator_factor(void)
{
    int r;

    // 当前token的类型
    switch (calculator_tokenizer_token())
    {
    // 数字(终结符)
    case TOKENIZER_NUMBER:
        // 将其由ASCII转为数字值
        r = calculator_tokenizer_num();
        // 根据语法规则匹配当前token
        calculator_accept(TOKENIZER_NUMBER);
        break;
    // 左括号
    case TOKENIZER_LPAREN:
        calculator_accept(TOKENIZER_LPAREN);
        // 将括号里的值当作一个新的表达式, 递归计算(递归是从函数expr()开始的)
        r = calculator_expr();
        // 当括号里的表达式处理完毕后下一个token一定是右括号
        calculator_accept(TOKENIZER_RPAREN);
        break;
...
/*
 *  函数名：   static int calculator_term(void)
 *  输入参数： 无
 *  返回值：   计算结果
 *  函数作用： 求第二级优先级(乘除)表达式的值
*/
static int calculator_term(void)
{
    int f1, f2;
    token_t op;

    // 获取左操作数(因子)
    f1 = calculator_factor();
    // 获取操作符
    op = calculator_tokenizer_token();
...
/*
 *  函数名：   static int calculator_expr(void)
 *  输入参数： 无
 *  返回值：   计算结果
 *  函数作用： 求第一级优先级(加减)表达式的值
*/
static int calculator_expr(void)
{
    int t1, t2 = 0;
    token_t op;
...
/*
 *  函数名：   static void calculator_expr(void)
 *  输入参数： 输入的表达式字符串
 *  返回值：   无
 *  函数作用： 词法分析器初始化
*/
static void calculator_tokenizer_init(char *expr)
{
