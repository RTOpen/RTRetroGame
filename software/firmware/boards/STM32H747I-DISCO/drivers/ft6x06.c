...
static int32_t FT6X06_DetectTouch(void);
...
int32_t FT6X06_GestureConfig(FT6X06_Gesture_Init_t *GestureInit)
{
  ...
  ret = ft6x06_radian_value((uint8_t)GestureInit->Radian);
  ret += ft6x06_offset_left_right((uint8_t)GestureInit->OffsetLeftRight);
  ret += ft6x06_offset_up_down((uint8_t)GestureInit->OffsetUpDown);
  ret += ft6x06_disatnce_left_right((uint8_t)GestureInit->DistanceLeftRight);
  ret += ft6x06_distance_up_down((uint8_t)GestureInit->DistanceUpDown);
  ret += ft6x06_distance_zoom((uint8_t)GestureInit->DistanceZoom);
...
int32_t FT6X06_GetState(FT6X06_State_t *State)
{
  ...
  State->TouchDetected = (uint32_t)FT6X06_DetectTouch();
  if(ft6x06_read_reg(FT6X06_P1_XH_REG, data, (uint16_t)sizeof(data)) != FT6X06_OK)
  {
    ret = FT6X06_ERROR;
  }
  else
...
{  
  return ft6x06_gest_id(GestureId);
}
...
{
  return ft6x06_g_mode(FT6X06_G_MODE_INTERRUPT_TRIGGER);
}
...
{
  return ft6x06_g_mode(FT6X06_G_MODE_INTERRUPT_POLLING);
}
...
static int32_t FT6X06_TS_Calibration(FT6X06_Object_t *pObj)
{
  ...
  if(ft6x06_dev_mode_w(FT6X06_DEV_MODE_FACTORY) != FT6X06_OK)
  {
    ret = FT6X06_ERROR;
  }/* Read back the same register FT6X06_DEV_MODE_REG */
  else if(ft6x06_dev_mode_r(&read_data) != FT6X06_OK)
  {
    ret = FT6X06_ERROR;
  }
  else
  {
    ...
    else
    {
      ...
      read_data= 0x04;
      if(ft6x06_write_reg(FT6X06_TD_STAT_REG, &read_data, 1) != FT6X06_OK)
      {
        ret = FT6X06_ERROR;
      }
      else
      {
        ...
        {
          if(ft6x06_dev_mode_r(]&read_data) != FT6X06_OK)
          {
            ret = FT6X06_ERROR;
            break;
          }
...
static int32_t FT6X06_DetectTouch(void)
{
  ...
  if(ft6x06_td_status(&nb_touch) != FT6X06_OK)
  {
    ret = FT6X06_ERROR;
  }
  else
